#!/usr/bin/env bash
# AIDLC Design Review Hook - Windows Bash Installer (Git Bash/WSL)
# Version: 1.0
...
if [[ -n "${MSYSTEM:-}" ]] || [[ "$(uname -s)" == MINGW* ]] || [[ "$(uname -s)" == MSYS* ]]; then
    RUNNING_IN="Git Bash"
elif grep -qsi "microsoft" /proc/version 2>/dev/null || grep -qsi "wsl" /proc/version 2>/dev/null; then
    RUNNING_IN="WSL"
else
    RUNNING_IN="Unknown"
fi
...
print_header() {
    ...
    echo -e "${BLUE}║              Windows (Git Bash/WSL) Version                    ║${NC}"
    echo -e "${BLUE}║                   Version 1.0                                  ║${NC}"
    echo -e "${BLUE}║                                                                ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${BLUE}Running in: $RUNNING_IN${NC}"
    echo ""
}
...
check_dependencies() {
    ...
    # Windows-specific: Check line ending handling
    if [[ "$RUNNING_IN" == "Git Bash" ]]; then
        print_info "Git Bash detected - checking line ending configuration..."
        local core_autocrlf=$(git config --get core.autocrlf 2>/dev/null || echo "not set")
        if [[ "$core_autocrlf" == "true" ]]; then
            print_warning "core.autocrlf is set to 'true' - this may cause issues with bash scripts"
            print_warning "Recommend: git config --global core.autocrlf input"
        else
            print_success "Line ending configuration OK"
        fi
    fi
    ...
    if command -v yq &> /dev/null; then
        ...
        if [[ "$RUNNING_IN" == "Git Bash" ]]; then
            echo "  To install: Download from https://github.com/mikefarah/yq/releases"
        else
            echo "  To install in WSL: sudo wget -qO /usr/local/bin/yq https://github.com/mikefarah/yq/releases/latest/download/yq_linux_amd64"
        fi
    fi
    ...
    if command -v python3 &> /dev/null; then
        ...
        print_warning "Python not found (optional - will use defaults)"
    fi
...
create_config() {
    ...
    # Use Unix-style line endings (LF) even on Windows
    cat > "$config_file" <<EOF
...
install_files() {
    ...
    if command -v dos2unix &> /dev/null; then
        print_info "Converting line endings to Unix format..."
        dos2unix "${TARGET_DIR}"/lib/*.sh 2>/dev/null || true
        dos2unix "${TARGET_DIR}/hooks/pre-tool-use" 2>/dev/null || true
...
run_validation() {
    ...
    if command -v yq &> /dev/null; then
        ...
        if python -c "import yaml; yaml.safe_load(open('${TARGET_DIR}/review-config.yaml'))" > /dev/null 2>&1; then
            print_success "Configuration file is valid YAML"
        else
            print_error "Configuration file has YAML syntax errors"
            return 1
...
show_instructions() {
    ...
    if [[ "$RUNNING_IN" == "Git Bash" ]]; then
        echo -e "${YELLOW}Git Bash Note:${NC}"
        echo "  If you encounter 'command not found' errors, check line endings:"
...
main() {
    ...
    if [ ! -f "$SOURCE_DIR/hooks/pre-tool-use" ]; then
        ...
        print_error "Example: ./tool-install/install-windows.sh"
        exit 1
    fi
